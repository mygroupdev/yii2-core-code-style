...
- ДОЛЖНЫ использоваться только открывающие теги `<?php`;
...
- Имена свойств ДОЛЖНЫ быть объявлены используя `snake_case`;
...
- В PHP коде ДОЛЖНЫ использоваться теги `<?php ?>`; НЕ ДОЛЖНЫ использоваться другие теги, такие как `<?`;
...
- Закрытые переменные должны иметь имя вида `$_var_name`;
- Общедоступные члены класса и отдельные переменные должны использовать `$snake_case`;
...
class Foo
{
    public $public_prop_1;
    public $public_prop_2;
...
`@brief`, `@param`, `@var`, `@property` и `@return` должны описывать типы `bool`, `int`, `string`, `array` или `null`. Вы можете использовать и имена классов, например `Model` или `ActiveRecord`. Для типизированных массивов используйте `ClassName[]`.
...
<?php
/**
 * @brief Returns the errors for all attribute or a single attribute.
...
```php
/**
 * @brief Component is the base class that provides the *property*, *event* and *behavior* features.
 *
...
```php
/**
 * @brief Returns the list of attached event handlers for an event.
